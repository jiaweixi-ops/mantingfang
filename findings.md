...
- The execution chain remains deterministic: current-frame local category geometry -> pure plan -> existing guarded Win32 SendInput adapter -> fresh local postcondition. Qwen, telemetry, and the Mono debugger stay unused.
- The V2.4C implementation passed `146` tests, Python `compileall`, and `git diff --check` before any real input. The live command is still unrun and must wait for a manual ROOT_OPEN setup plus exact game foreground.
- First zero-input dry-run stopped with `category plan rejects non-current-frame category evidence`: `parse_build_menu_snapshot()` intentionally condensed evidence and dropped the observer provenance marker. Fix: explicitly carry the deterministic source marker into the resulting immutable snapshot; rerun dry-run only after focused tests.
- V2.4C dry-run then passed using fresh WGC geometry (Song HWND `134730`, PID `32776`, `1280x960`, DPI 96), eight fresh local category bboxes, and local close confidence `0.98066`; Qwen and input use were both zero in dry-run.
- V2.4C real execution passed: exactly one left click targeted `build_category_tab_07` at normalized `[0.7921875, 0.746875]` / screen `[1015, 804]`. Cursor verification, move/down/up `SendInput` return counts, exact foreground HWND/PID, and geometry all passed. The fresh post-click WGC frame had `CATEGORY_OPEN`, eight local BUILD_OPTION slots, a changed controls fingerprint, and a responsive Song window. The run disarmed live input in `finally`; no retry, keyboard, map/option click, Qwen, telemetry, debugger, save write, or memory write occurred.
- V2.4D will keep geometry and semantic data separate. The repository has no existing OCR/template helper, so local semantic inference remains conservative and the only permitted fallback is one strict no-retry `qwen3.8-flash` call over current card crops.
- Initial V2.4D semantic tests exposed and fixed two fail-closed issues before any real call: missing available resources now return `UNKNOWN` without indexing absent keys, and negative model costs have an explicit rejection error.
- The first real V2.4D attempt stopped before the Qwen HTTP call because the new montage canvas allocation omitted the column count. The frame itself was valid (`1280x960`, RGBA length `4915200`); the canvas allocation was corrected and the call counter now increments only after a valid montage is built.
- Final V2.4D evidence: frame `6f3947cdd5034dbf93538b7d79e4007d`, current geometry-bound options 01-08, labels `茶坊/脚店/斗香场/戏台子/集市/客店/酒楼/燕馆歌楼`, model confidence `0.95`, 8 lock states reported as `false`, and costs unresolved. Geometry remained the detector's current-frame geometry; no model coordinates were accepted.
