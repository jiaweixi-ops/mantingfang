...
class QwenConfigurationError(RuntimeError):
    """Raised when the Qwen provider is not configured safely."""
...
class QwenClient:
    """Standard-library Qwen OpenAI-compatible chat/vision client.

    The provider is intentionally independent from the Governor and accepts
    the same small `complete_json`/`analyze_image_json` boundary used by the
    existing strategy and perception code. API keys are never included in
    ...
    def __init__(
        ...
        self.last_usage: QwenUsage | None = None
    ...
    def complete_json(
        ...
        *,
        ...
        if not chosen_model:
            raise QwenConfigurationError("a Qwen model must be configured")
        payload = {
            "model": chosen_model,
            "messages": messages,
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt >= self.max_retries:
                    raise QwenRequestError(str(exc), error_type=type(exc).__name__) from exc
            self.sleep_fn(self.backoff_seconds * (2 ** attempt))
        if body is None:
            raise QwenRequestError(str(last_error or "Qwen request failed"), error_type="request_failed")
        ...
        try:
            ...
            raise QwenRequestError("Qwen returned a non-JSON response", error_type="invalid_json") from exc
