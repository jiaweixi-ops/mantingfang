...
def test_cli_exposes_v24d_read_only_semantic_command() -> None:
    args = build_parser().parse_args(["e2e-parse-build-options"])
    assert args.command == "e2e-parse-build-options"
    assert args.output_dir == "data/probe/V2.4D"


def test_cli_exposes_guarded_v24ef_roundtrip_command() -> None:
    args = build_parser().parse_args(["e2e-build-placement-roundtrip", "--confirm-live-placement"])
    assert args.command == "e2e-build-placement-roundtrip"
    assert args.confirm_live_placement is True
